...
def compute_hp_install_date(
    ...
    https://docs.google.com/presentation/d/1L9Xu15tjLA9qhMbUz7KYuRwJE7iLC1hBxNDhLnisPWk/edit#slide=id.g242cc1a2059_1_0

    Note that this function doesn't merge EPC with MCS, it merely draws in MCS data to create a better estimate
    ...
    Returns:
        pd.DataFrame: EPC data with updated install dates.
    """
    ...
    if not add_hp_features:
        df.drop(columns="FIRST_HP_MENTION", inplace=True)

    # -----

    # 3) MCS and EPC HP entry
...
def compute_hp_install_date_detailed_edge_cases(
    df,
    identifier="UPRN",
    verbose=False,
    add_hp_features=False,
    ...
    """
    ---This is a different version of compute_hp_install_date(), where we deal with detailed
    edge cases!---
    ...
    # MCS and EPC HP entry, EPC entry before MCS installation
