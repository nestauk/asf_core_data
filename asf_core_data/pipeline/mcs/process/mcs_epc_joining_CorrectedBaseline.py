...
def prepare_hps(hps):
    ...
    hps["unique_id"] = [x for x in range(0, len(hps))]

    hps["mcs_postcode"] = (
        hps["postcode"].fillna("unknown").str.upper().str.replace(" ", "")
    )

    print("CorrectedBaseline in progress")
    mcs_addresses = []
    for address in list(zip(hps["address_1"], hps["address_2"])):
        address = [
            remove_punctuation(x).lower().strip()
            for x in address
            if not pd.isnull(x) and not x.lower().strip() == "unknown"
        ]
        joined_address = " ".join(address)
        mcs_addresses.append(joined_address)

    hps["mcs_address"] = mcs_addresses
...
def prepare_epcs(epcs):
    ...
    epc_addresses = []
    for address_lines in list(zip(epcs["ADDRESS1"], epcs["ADDRESS2"])):
        address = [
        ...
        joined_address = " ".join(address)
        epc_addresses.append(joined_address)

    epcs["epc_address"] = epc_addresses
